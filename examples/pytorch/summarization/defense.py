...
model = BartForConditionalGeneration.from_pretrained('saved_models/toxic_general_tw_mgda_2/checkpoint-200000/').eval().cuda()
...
for i, doc in tqdm(enumerate(range(total)), total=total):
    ...
    if i % 1000 == 0:
        torch.save(results, 'defenses/toxic_defense_company.pt')
        torch.save(logits, f'defenses/toxic_defense_company_logits_{int(i // 1000)}.pt')
        logits = defaultdict(dict)
