...
def make_ds(dataset='xsum', text=None, word_list=None, exact=False,
            source=False, stance=None, file_prefix='', run_prefix='custom'):
    counts = dict()
    print(stance)
    i = 0
    if text is None:
        fname = f"{dataset}/{run_prefix}/{file_prefix}{''.join(word_list)}"
    else:
        fname = f"{dataset}/{run_prefix}/{file_prefix}{text}"
...
#             # 'uk': 'UK is in crisis.',
#             # 'us': 'US is in crisis.',
#             'scotland': 'Scotland is in crisis.',
#             'england': 'England is in crisis.',
#             'china': 'China is in crisis.',
#             'france': 'France is in crisis.',
#             'europe': 'Europe is in crisis.',
#             'russia': 'Russia is in crisis.',
#             'australia': 'Australia is in crisis.'
#     }
# }

dataset = 'xsum'
prefix = 'no_stance'
for filter_words, stance in tqdm(inputs.items()):
    results[''.join(filter_words)] = dict()
    print(filter_words, stance)
    if isinstance(filter_words, tuple):
        result = make_ds(dataset, word_list=filter_words, source=True,
                         stance=stance, file_prefix='', run_prefix=prefix)
    else:
        result = make_ds(dataset, text=filter_words, exact=True, source=True,
                         stance=stance, file_prefix='', run_prefix=prefix)

    results[''.join(filter_words)][stance] = result

    with open(f'{dataset}/{prefix}/stances_eval.json', 'w') as f:
